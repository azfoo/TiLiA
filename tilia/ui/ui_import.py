    ...
    if time_or_measure == "measure":
        ...
        return (
            ...
            [],
            {
                ...
                "time_or_measure": time_or_measure,
            },
        )

    return "success", [], {"timeline": timeline, "time_or_measure": "time"}
...
def on_import_from_csv(
    ...
    success, errors, args = _get_import_timeline_args(
        timeline_uis, tlkind, "time" if tlkind is TlKind.BEAT_TIMELINE else None
    )
    if success != "success":
...
def on_import_from_musicxml(
    ...
    success, errors, args = _get_import_timeline_args(
        timeline_uis, tlkind, "measure" if tlkind is TlKind.SCORE_TIMELINE else None
    )
    if success != "success":
        return success, errors
    ...
    try:
        ...
        tilia.errors.display(tilia.errors.INVALID_MUSICXML_ERROR, path)
...
def _get_by_time_or_by_measure_from_user() -> tuple[
    Literal["success", "failure"], Optional[Literal["time", "measure"]]
]:
    dialog = ByTimeOrByMeasure()
    if not dialog.exec():
        return "failure", None
    return "success", dialog.get_option()
